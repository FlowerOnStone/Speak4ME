...
import { StyleSheet, View, ActivityIndicator, Keyboard, TextInput } from 'react-native';
...
export class SearchBarAndroid extends Component {
    ...
    render() {
        ...
        const wrapper = { testID: 'RNE__SearchBar-wrapper', style: StyleSheet.flatten([
            {
        ...
        // let gg = () => (<Input containerStyle={{ borderRadius:25, backgroundColor:"blue" }}/>);
        let textInput = React.createElement(Input, Object.assign({ testID: 'RNE__SearchBar', renderErrorMessage: false }, attributes, { onFocus: this.onFocus, onBlur: this.onBlur, onChangeText: this.onChangeText, ref: (input) => {
            this.input = input;
        ...
        if (typeof this.props.dropShadow !== 'undefined')
        {
            textInput = React.createElement(DropShadow, this.props.dropShadow, textInput);
        }

        return (React.createElement(View, wrapper, textInput));
    }
}
